...
async def button(bot, update: CallbackQuery):
    ...
    if (update.from_user.id == update.message.reply_to_message.from_user.id) or g:
        ...
        if "status" in cb_data:
            await update.answer("Postion In Queue: {}\n\nTotal: {}".format(Queue_id.index(update.from_user.id) + 1, len(Queue)), show_alert=True)
        if "cancel" in cb_data:
            pos = Queue_id.index(update.from_user.id)
            Queue_id.remove(update.from_user.id)
            Queue.remove(Queue[int(pos)])
            await update.message.edit("Task removed from Queue")
